...
class GraphAgent_Plan_Reflect_vllm:
    def __init__(self,
        ...
        self.default_examples = PLAN_EXAMPLES[args.ref_dataset] if args.compound_strategy == 'plan_compound' else PLAN_ONLY_EXAMPLES[args.ref_dataset]
        ...
        #     self.enc = AutoTokenizer.from_pretrained(args.llm_version, trust_remote_code=True)
            ...
            try:
                self.enc = AutoTokenizer.from_pretrained(local_model_path, trust_remote_code=True)
                self.enc2 = self.enc
            except Exception as e:
                # 如果路径不对，再尝试用传进来的名字去下
                print(f"本地加载失败，尝试使用默认名称加载: {e}")
                self.enc = AutoTokenizer.from_pretrained("Qwen/Qwen2.5-7B-Instruct", trust_remote_code=True)
                self.enc2 = self.enc
        else:
            # 1. 打印一下到底传进来了什么，死个明白
            print(f"[Debug] 检测到未知的模型名称: {args.llm_version}，正在强制使用本地 Qwen 配置...")
            from transformers import AutoTokenizer
            # 2. 强制指定本地模型路径 (请确保这个路径是对的！)
            local_model_path = "/root/Graph-Counselor/Graph-Counselor/models/qwen/Qwen2.5-7B-Instruct"
            self.enc = AutoTokenizer.from_pretrained(local_model_path, trust_remote_code=True)
            self.enc2 = self.enc
            print("false")
    ...
    def run(self, question, answer,dynamic_examples=None, reset = True) -> None:
        if reset:
            self.__reset_agent()
            self.round_n = 1
            ...
            self.answer_first = ""
        if dynamic_examples:
            self.examples = dynamic_examples
        else:
            self.examples = self.default_examples
        ...
        if self.judge_correct == 'groundtruth':
            while not self.is_correct() and self.round_n <= int(self.max_reflect):  #/is_correct_LLM
                ...
                while not self.is_halted() and not self.is_finished():
                    print(f"🔄 [DEBUG-GT] 正在执行 Step {self.step_n}...")
                    self.step()
            ...
            while not self.is_correct_LLM() and self.round_n <= int(self.max_reflect):  #/is_correct_LLM
                ...
                self.__reset_agent()
                while not self.is_halted() and not self.is_finished():
                    print(f"🔄 [DEBUG-LLM] 正在执行 Step {self.step_n}...")
                    self.step()
    ...
    def step(self) -> None:
        ...
        # self.scratchpad += ' ' + self.prompt_agent()
        # print(self.scratchpad.split('\n')[-1])
        combined_response = self.prompt_agent()
        ...
        # print(self.scratchpad.split('\n')[-1])
        self.scratchpad += ' ' + combined_response
        print(f"🧠 [Step {self.step_n}] Full Generation: {combined_response}") # Debug打印
        if f"Action {self.step_n}:" in combined_response:
            action_part = combined_response.split(f"Action {self.step_n}:")[-1].strip()
        elif "Action:" in combined_response:
            action_part = combined_response.split("Action:")[-1].strip()
        else:
            # 如果模型忘了写 Action: 前缀，尝试直接解析最后一行 (风险较大，建议在 Prompt 里强调格式)
            action_part = combined_response.split('\n')[-1].strip()
        action = action_part
        ...
        print(f"Executing Action: {action_list}")
    ...
    def prompt_agent(self) -> str:
        ...
        elif self.llm_version in ['../model/Qwen2.5-7B-Instruct','Qwen2.5-7B-Instruct']:
            payload = {
                ...
                "top_p": 0.9,
                "n": 1,
                # ✅ 正确：允许模型换行，直到遇到 Observation 才停
                "stop": ["Observation", "Observation:", f"Observation {self.step_n}:"]
            }
            
            try:
                # ✅ 必须保留 timeout，因为生成的文本变长了
                ...
                if response.status_code == 200:
                    ...
                    # ✅ 清理逻辑：去掉可能残留的 stop token
                    # 有时候 vLLM 会保留 stop token 的一部分，手动切掉最稳妥
                    stop_str = f"Observation {self.step_n}:"
                    if stop_str in mid_ans:
                        mid_ans = mid_ans.split(stop_str)[0]
                    
                    # ⚠️ 关键修改：直接返回完整文本，不要调用 Qwen_format_step
                    return mid_ans.strip()
    ...
    def prompt_reflection(self) -> str:
        if self.reflect_version in ['gpt-3.5-turbo', 'gpt-4', 'gpt-3.5-turbo-1106', 'gpt-3.5-turbo-16k']:
            ...
            print('*******************************************************************')
            return reflect_format_step(mid_ans)
        elif self.reflect_version in ['../model/Qwen2.5-7B-Instruct','Qwen2.5-7B-Instruct']:
            ...
            if total_tokens < 31700:
                try:
                    print(f"🧠 [DEBUG] 正在发送反思请求 (Reflect)...")
                    response = requests.post(self.api_url3, json=payload,timeout=600)
                    if response.status_code == 200:
                        result = response.json()
                        mid_ans = result["choices"][0]["text"]
                        print(f"✅ [DEBUG] 反思成功完成")
                        return reflect_format_step(mid_ans)
                    else:
                        print(f"❌ [错误] vLLM 反思 API 报错: {response.status_code}, {response.text}")
                        # 报错时返回空字符串，避免程序直接崩溃退出
                        return ""
                    # raise RuntimeError(f"vLLM API Error: {response.status_code}, {response.text}")
                except requests.exceptions.Timeout:
                    print("❌ [严重错误] 反思请求超时！跳过此步。")
                    return ""
                except Exception as e:
                    print(f"❌ [网络错误] 反思连接失败: {e}")
                    return ""
                # return reflect_format_step(mid_ans)
            else:
                print("⚠️ [警告] Token 过长，跳过反思。")
                return ""
