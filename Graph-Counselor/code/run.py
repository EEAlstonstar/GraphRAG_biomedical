...
from tools.dynamic_retriever import DynamicFewShotRetriever
...
#方向3
parser.add_argument("--few_shot_bank_file", type=str, default="few_shot_bank.json", help="Filename of the few-shot bank under args.path")
parser.add_argument("--use_dynamic_fewshot", type=bool, default=True, help="Enable dynamic retrieval")
parser.add_argument("--dynamic_k", type=int, default=3, help="Number of examples to retrieve")
...
def main():
    ...
    if args.use_dynamic_fewshot:
        # 自动拼接路径：../data/processed_data/biomedical/few_shot_bank.json
        bank_full_path = os.path.join(args.path, args.few_shot_bank_file)
        local_model_path = "/root/Graph-Counselor/Graph-Counselor/models/AI-ModelScope/all-mpnet-base-v2"
        embedder_model = local_model_path if os.path.exists(local_model_path) else "sentence-transformers/all-mpnet-base-v2"
        if os.path.exists(bank_full_path):
            print(f"🚀 [Dynamic Few-Shot] Initializing retriever from: {bank_full_path}")
            # 注意：复用 args.embedder_name (sentence-transformers/all-mpnet-base-v2)
            # 如果你有显卡，可以把 device='cuda' 写死或者传参
            fewshot_retriever = DynamicFewShotRetriever(
                bank_path=bank_full_path, 
                model_name=embedder_model
            )
        else:
            print(f"⚠️ [Dynamic Few-Shot] Warning: Bank file not found at {bank_full_path}. using static examples.")
    #------------------------
    ...
    for i in tqdm(range(len(contents))):
        # ===【修改 4：先检索，再运行】===
        dynamic_examples_str = None
        if fewshot_retriever:
            try:
                # 针对当前问题 contents[i]['question'] 检索 top-k
                dynamic_examples_str = fewshot_retriever.retrieve(
                    contents[i]['question'], 
                    k=args.dynamic_k
                )
        ...
        # 将 dynamic_examples 传递给 agent (注意：这需要你之前的 GraphAgent_Plan_Reflect_vllm.py 已经改好了！)
        agent.run(contents[i]['question'], contents[i]['answer'], dynamic_examples=dynamic_examples_str)
        # ===============================
